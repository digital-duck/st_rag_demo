...
if st.session_state.viewing_history and st.session_state.history_session_id:
    ...
    loaded_messages, session_metadata = st.session_state.history_manager.load_chat_history(st.session_state.history_session_id)
    
    if loaded_messages:
        # Create a container for the history view
        st.subheader(f"Chat History for {st.session_state.history_session_id} ({len(loaded_messages)} messages)")
...
# Only show normal interface if not viewing history
