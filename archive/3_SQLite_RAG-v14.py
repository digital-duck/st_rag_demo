...
from utils.ui_helpers import (
    ...
    add_assistant_message, process_chat_message
)
from utils.sidebar import display_sidebar
from utils.visualization import generate_visualization, format_sql_for_display
    ...
    with st.spinner("Executing query..."):
        try:
            ...
            if isinstance(result, pd.DataFrame) and not result.empty:
                    ...
                    with st.spinner("Generating visualization..."):
                        try:
                            ...
                            if isinstance(fig, go.Figure):
                                ...
                                # Fallback to auto visualization if custom fails
                                st.write("Attempting automatic visualization...")
                                fig = generate_visualization(result, chart_type="auto")
                                if isinstance(fig, go.Figure):
                                    st.plotly_chart(fig, use_container_width=True)
                            ...
                            st.info("Visualization requires appropriate numeric and/or categorical columns.")
                            
                            # Display column types for debugging
                            st.write("Column types:")
                            col_types = pd.DataFrame({
                                'Column': result.columns,
                                'Type': [str(result[col].dtype) for col in result.columns]
                            })
                ...
                add_assistant_message("sqlite", "sqlite_messages", "sqlite_session_id", 
                                     result_summary, query=sql_query)
