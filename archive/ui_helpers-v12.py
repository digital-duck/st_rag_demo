...
def display_example_questions(app_type, messages_key, examples):
    """
    Display example question buttons that will trigger the LLM
    ...
    for i, example in enumerate(examples[:3]):  # Limit to first 3 examples
        with cols[i]:
            if st.button(example):
                add_user_message(app_type, messages_key, example)
...
def add_assistant_message(app_type, messages_key, session_id_key, answer_text, query=None, data=None):
    ...
    Args:
        ...
        messages_key (str): Key for messages in session state
        session_id_key (str): Key for session ID in session state
        answer_text (str): The answer text to add
        query (str, optional): SQL query for SQLite responses
        data (pd.DataFrame, optional): Data for SQL responses
    """
    assistant_message = {
        "role": "assistant", 
        "content": answer_text,
    ...
    # Add query if provided (for SQLite)
    if query:
        assistant_message["query"] = query
    
    # Add data dictionary if provided (for SQLite)
    ...
    # Save chat history
    save_chat_history(app_type, session_id_key, messages_key, f"{app_type}_chatbot")
...
def display_history_view(history_manager):
    ...
    loaded_messages, session_metadata = history_manager.load_chat_history(session_id)
