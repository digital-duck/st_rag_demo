...
def process_chat_message(prompt):
    ...
    with st.chat_message("assistant"):
        debug_info = {
            "prompt": prompt,
            "stage": "starting",
            "is_debug_mode": st.session_state.debug_mode,
            "timestamp": datetime.now().isoformat()
        }
        ...
        with st.spinner("Thinking..."):
            try:
                ...
                try:
                    ...
                    if not generated_query.startswith("Error"):
                        ...
                        if isinstance(query_result, pd.DataFrame):
                            ...
                            # Modify the answer to NOT include the table output
                            ...
                            # Don't add the table text here, since we'll display it with st.dataframe
                            
                            # Display the answer text
                            st.markdown(answer)
                            
                            # Then add a heading and show the data
                            st.write("Here's the result:")
                            st.dataframe(query_result)
                            
                            # [Rest of visualization code remains the same]
                            
                            # When adding to chat history, use this same text without the table
                            response_with_data = {
                                "answer": answer,
                ...
                # When adding to chat history, pass the debug_info
                response_with_data = {
                    "answer": answer,
                    "query": generated_query,
                    "data": query_result,
                ...
                add_assistant_response_with_context(response_with_data, debug_info)
...
def add_assistant_response_with_context(response, debug_info=None):
